...
#
# Day to day, an admin does this from the browser: /admin, or the "Guest list"
...
case "${1:-list}" in
  list)
    psql <<<"SELECT email, name, is_admin, invited_on, last_login_at
  ...
  remove)
    ...
    # revoked user's next click sends them to the login page, session or no session.
