...
this mapping as-is. Anything at or above 0.9 is committed automatically \
without review. Creating new book/person/series rows from clean, unambiguous \
metadata when there are no plausible candidates IS a high-confidence action \
(>= 0.9) — a sparse catalog is normal, not a reason to hedge. Reserve lower \
confidence for genuine ambiguity: a candidate that might or might not be the \
same work or person, conflicting metadata, or uncertain series placement that \
...
def find_cover(asset_type: str, asset_id: int) -> Path | None:
    covers_dir = os.environ.get("RESOLVER_COVERS_DIR")
    if not covers_dir:
        return None
