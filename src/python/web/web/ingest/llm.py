...
# --- correction: fix a book already in the catalog ---------------------------
#
# The resolver above answers "what is this file?". This answers a different
# question -- "what did we get wrong about this book?" -- and it is asked from
# the book's own page, with no asset in hand and nothing being added.


class BookCorrection(BaseModel):
    """The corrected record for one existing book.

    Every field is optional and None means "leave this alone", so a correction
    about the publication date cannot quietly restate -- and thereby change --
    the title. That also means a field cannot be *cleared* this way; removing a
    book from a series is a deletion, not a correction, and has no route here.
    """

    title: str | None
    series: SeriesRef | None
    series_position: int | None
    publication_date: str | None  # ISO; the work's FIRST publication
    language: str | None
    # None leaves the credits alone; a list REPLACES them, in order, which is
    # what "you got the author wrong" means.
    authors: list[PersonRef] | None
    confidence: float
    notes: str


CORRECT_SYSTEM_PROMPT = """You are the catalog editor for a personal \
ebook/audiobook library. You are given one book's current catalog record, an \
admin's plain-language complaint about it, and short lists of candidate people \
and series already in the catalog. Return the corrected record.
...
knowledge of the work to supply the right value — and if you do not know it, \
leave the field null and say so in notes rather than guessing.
- authors: null leaves the credits untouched. A list REPLACES the whole credit \
list in the order given, so include every author the book should end up with, \
not just the one being added or removed. Link an existing candidate person \
whenever the name is the same person (initials and punctuation vary: \
"J.R.R. Tolkien" / "JRR Tolkien"); create only when no candidate is that \
person. Credit the name on the cover, pseudonym or not.
- series: link a candidate when the book belongs to a series already in the \
catalog; create only when it is genuinely absent. Positions are whole numbers; \
an interstitial novella gets membership with a null position.
- title: the work's core title only. Series names, "Book N" markers and genre \
taglines ("A Novel", "A LitRPG Adventure") belong in their own fields or \
nowhere.
- publication_date: the work's FIRST publication, ISO YYYY-MM-DD, or \
YYYY-01-01 if you only know the year. Not the date of this edition or \
reprint.
- language: BCP-47 ('en', 'pt', 'pt-PT').
- confidence: the probability (0.0-1.0) that the admin will accept this \
correction as-is. They review it before anything is written, so an honest low \
score is more useful than a confident wrong one.
- notes: one or two short sentences saying what you changed and why — and, if \
you could not act on the correction, saying that plainly.
"""
...
def correct(
    ...
    candidates: dict[str, Any],
    instruction: str,
) -> tuple[BookCorrection, dict[str, int]]:
    """One structured call: the book as it stands, plus what is wrong with it."""
    payload = {
        "current_record": book,
        "candidates": candidates,
        "admin_correction": instruction,
        "today": datetime.date.today().isoformat(),
    }
    response = client.messages.parse(
        model=model_id(),
        ...
        output_format=BookCorrection,
    )
    return _parsed(response, "correct")


def to_correction(bc: BookCorrection) -> dict[str, Any]:
    """Map the model's corrected record to the action dict apply.py executes.

    A key is present only when that field is actually being changed, so "absent
    means unchanged" survives the trip through the browser and back -- which
    matters, because the browser is where the correction lives until it is
    accepted.
    """
    out: dict[str, Any] = {}
    if bc.title:
        out["title"] = bc.title
        out["sort_title"] = sort_title(bc.title)
    series_action = _series_action(bc.series)
    if series_action is not None:
        out["series"] = series_action
    if bc.series_position is not None:
        out["series_position"] = bc.series_position
    published = parse_pub_date(bc.publication_date)
    if published is not None:
        out["publication_date"] = published
    language = norm_language(bc.language)
    if language is not None:
        out["language"] = language
    if bc.authors is not None:
        out["authors"] = [_person_action(a) for a in bc.authors]
    return out
