...
-- One identity for anyone who authors or narrates. Roles are expressed by which
-- column/table references a person (book_authors.author_id, m4b_narrators.narrator_id),
-- so a person who both writes and narrates (e.g. Travis Baldree) is a single row.
CREATE TABLE IF NOT EXISTS people (
    id            SERIAL PRIMARY KEY,
    name          TEXT NOT NULL,
    sort_name     TEXT NOT NULL,
    disambiguator TEXT                        -- human-facing note when two people share a name; NULL otherwise
);

CREATE INDEX IF NOT EXISTS idx_people_sort_name ON people(sort_name);

CREATE TABLE IF NOT EXISTS author_pseudonyms (
    pseudonym_id    INT NOT NULL REFERENCES people(id) ON DELETE CASCADE,
    author_id       INT NOT NULL REFERENCES people(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS books (
    ...
    series_position  INT                      -- whole-number position in series; NULL if unnumbered/interstitial
);
...
-- Narrator is a property of the audio edition (the m4b), not the abstract book:
-- two m4bs of one book can carry different narrators. Curation join, references people.
CREATE TABLE IF NOT EXISTS m4b_narrators (
    m4b_id      INT      NOT NULL REFERENCES m4bs(id) ON DELETE CASCADE,
    narrator_id INT      NOT NULL REFERENCES people(id) ON DELETE CASCADE,
